...
//    private TextView ssdText;
    ...
    private List<Menuitem> menuitems2 = new ArrayList<Menuitem>();
    private ListView menulist;
    private ListView menulist2;
    private ToggleButton autorefreshbtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        menulist2 = (ListView) findViewById(R.id.menu_item2);
        ...
        menuitems2.add(info);
        menuitems2.add(quilt);
        MenuAdapter adapter2 = new MenuAdapter(WeatherActivity.this, R.layout.menu, menuitems2);
        menulist2.setAdapter(adapter2);
        slidermenu = (LinearLayout) findViewById(R.id.slidermenu);
        initViews();
        autorefreshbtn = (ToggleButton) findViewById(R.id.autorefresh_btn);
        GPSbtn = (ToggleButton) findViewById(R.id.GPS_btn);
        initsetmenu();
        ...
        menulist2.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Menuitem menuitem = menuitems2.get(position);
                String menuname = menuitem.getContext();
                switch (menuname) {
                    case "关于":
        ...
        autorefreshbtn.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                set(WeatherActivity.this,"autorefresh",isChecked);
            }
        });
        GPSbtn.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                set(WeatherActivity.this,"GPS",isChecked);
            }
        });
...
//        menuItems.add(quilt);
    ...
    private void initsetmenu(){
        boolean isautorefresh=get(this,"autorefresh",true);
        autorefreshbtn.setChecked(isautorefresh);
        boolean isGPS=get(this,"GPS",true);
        GPSbtn.setChecked(isGPS);
    }
...
//        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_share:
                Toast.makeText(WeatherActivity.this, "分享暂不可用", Toast.LENGTH_SHORT).show();
                break;
...
//        ssdText.setText("舒适度："+prefs.getString("ssd_text","")+"  ");
    ...
    public static boolean get(Context context, String name, boolean defaultValue) {
        final SharedPreferences setprefs = PreferenceManager.getDefaultSharedPreferences(context);
        boolean value = setprefs.getBoolean(name, defaultValue);
        return value;
    }
